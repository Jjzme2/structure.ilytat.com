...
**Vulnerability:** Authorization bypass via unverified email claims.
**Learning:** When authorizing users based on hardcoded email lists in identity providers like Firebase, relying solely on `user.email` allows account takeover if a malicious user creates an unverified account with a whitelisted email.
**Prevention:** Always explicitly check `user.email_verified === true` alongside the email address when performing authorization checks based on email lists.
