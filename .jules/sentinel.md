...
## 2026-02-07 - Unverified Email Account Takeover in Admin Auth
**Vulnerability:** Admin access could be granted based solely on matching a hardcoded email list without verifying ownership of the email.
**Learning:** Identity providers like Firebase Auth allow creating accounts with any email address, even if not verified. If authorization relies on email matching without ensuring `user.email_verified === true`, a malicious user can create an account with an admin's email and gain elevated privileges.
**Prevention:** Always verify `user.email_verified === true` when using custom email lists for authorization logic.
