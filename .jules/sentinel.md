...
## 2026-02-07 - Admin Account Takeover via Unverified Email
**Vulnerability:** `requireAdmin` allowed access based on hardcoded email addresses without verifying if the email was actually verified by the user.
**Learning:** An attacker could create an account with a hardcoded admin email (e.g. `admin@ilytat.com`), and if the identity provider doesn't enforce email verification at signup, the attacker would gain admin privileges without ever proving ownership of the email.
**Prevention:** Always explicitly check `user.email_verified === true` on the decoded token when authorizing users based on email address.
