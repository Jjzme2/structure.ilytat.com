...
## 2024-03-07 - Information Exposure in API Error Responses
**Vulnerability:** The `/api/dates/public` endpoint exposed raw `error.message` details in its HTTP 500 response (`statusMessage: \`Internal Server Error: ${error.message || 'Unknown error'}\``).
**Learning:** Returning unhandled exception messages to the client can leak sensitive backend operational details, database schema information, or third-party dependency states to unauthenticated users.
**Prevention:** Always log full error details server-side (`console.error`) but return generic, safe messages (e.g., `'Internal Server Error'`) in the `statusMessage` or response body when throwing `createError` to the client.
