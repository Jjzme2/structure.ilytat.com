...
## 2025-02-28 - Unverified Email Admin Authorization Bypass
**Vulnerability:** A hardcoded admin email array allowed users to gain god-mode privileges solely if their email matched, without checking if they actually verified ownership of that email address.
**Learning:** Hardcoding email addresses for administrative access creates a privilege escalation risk if the identity provider (like Firebase Auth) allows account creation with arbitrary emails and doesn't enforce email verification by default.
**Prevention:** Always ensure `user.email_verified === true` is checked on the decoded token when performing authorization logic based on email string matching.
