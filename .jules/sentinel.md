...
## 2026-02-07 - Authorization Bypass via Unverified Emails
**Vulnerability:** The `requireAdmin` auth utility checked if a user's email matched a hardcoded list of admin emails, but failed to verify if the email address was actually validated by the identity provider (`user.email_verified === true`).
**Learning:** Checking an email claim alone is insufficient for authorization if the identity provider allows users to sign up with unverified emails. An attacker could register an account using a known admin email and bypass authorization.
