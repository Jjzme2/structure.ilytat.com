...
**Vulnerability:** Authorization logic relied on checking hardcoded email addresses without confirming `email_verified: true` on the identity provider token.
**Learning:** If an identity provider (like Firebase Auth) allows account creation with any email without enforcing verification, an attacker can create an account using a victim's email address and bypass domain/email-based access controls.
**Prevention:** Always verify `user.email_verified === true` when making authorization decisions based on a user's email address rather than their unique ID.
