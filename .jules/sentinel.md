...
**Vulnerability:** The `requireAdmin` utility in `server/utils/auth.ts` granted admin access based on a list of hardcoded emails without checking if the email address was verified.
**Learning:** Checking for email addresses without verifying the `email_verified` claim allows malicious users to bypass authorization by creating an unverified account with a targeted email address.
