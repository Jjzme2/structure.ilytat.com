...
## 2026-03-10 - Stop Error Detail Exposure
**Vulnerability:** `server/api/dates/public.get.ts` exposes internal `error.message` strings directly to the client via `createError`.
**Learning:** Exposing raw error messages or stack traces can leak sensitive database paths, architecture details, or credentials to attackers (Information Exposure).
**Prevention:** Always log detailed errors server-side, but return generic, sanitized messages (e.g., "Internal Server Error") in API responses.
