...
## 2026-02-07 - Auth Bypass via Unverified Hardcoded Emails
**Vulnerability:** `server/utils/auth.ts` allowed admin access based purely on matching `user.email` against a hardcoded list, without verifying `user.email_verified === true`.
**Learning:** Identity providers like Firebase Auth allow users to create accounts with any email address (even if they don't own it) unless email verification is enforced. Checking only `user.email` permits account takeover/privilege escalation by malicious actors who register an admin's email.
**Prevention:** Always explicitly verify `user.email_verified === true` on the decoded token when authorizing users based on hardcoded or custom email lists.
