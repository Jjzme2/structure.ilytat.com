...
## 2025-02-05 - Missing Email Verification on Hardcoded Auth
**Vulnerability:** Hardcoded administrative emails were granted access based solely on their email string without verifying that the email actually belongs to the user.
**Learning:** Any user can register an account with an arbitrary email in Firebase Auth. Without enforcing `email_verified`, a malicious user can trivially impersonate hardcoded admins and bypass authorization.
**Prevention:** When using hardcoded emails or custom domain checks for authorization, always enforce `email_verified === true` on the decoded token.
