...
## 2026-04-12 - Prevent IDOR in Document Storage
**Vulnerability:** Document upload, list, and download endpoints used a global `documents/` path, allowing any authenticated user to potentially list and download other users' documents by guessing the key (IDOR).
**Learning:** Storing all user files in a flat structure without ownership verification during access or listing exposes sensitive data.
**Prevention:** Always scope user uploads to specific prefixes (e.g., `documents/users/${uid}/`) and enforce these prefixes on list and download endpoints.
