...
## 2025-02-28 - Enforce email verification for implicit authorizations
**Vulnerability:** Authorization bypass via unverified email accounts.
**Learning:** Hardcoded email checks or domain-based authorization without verifying the email address could allow attackers to register fake accounts to gain unauthorized access.
**Prevention:** Always check `user.email_verified === true` (or `user.emailVerified === true` for `UserRecord`s) when relying on the `email` field for authorization.
