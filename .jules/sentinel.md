...
## 2026-02-07 - Information Leakage in API Error Responses
**Vulnerability:** API endpoints were leaking internal error details (`e.message`, `error.message`) to the client via Nuxt's `createError`.
**Learning:** Exposing internal error messages can provide attackers with insights into the system's architecture, dependencies, or database structures, aiding further attacks.
**Prevention:** Always log detailed error information server-side for debugging, but return generic, safe messages like 'Internal Server Error' to the client.
