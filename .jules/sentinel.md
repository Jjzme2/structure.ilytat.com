...
## 2025-02-18 - Missing Email Verification for Hardcoded Admins
**Vulnerability:** Hardcoded admin emails bypass email verification check
**Learning:** Checking for email matches without checking email_verified status allows attackers to create an unverified account with the admin email address to spoof admin access.
**Prevention:** Always verify `user.email_verified === true` when validating against hardcoded or custom lists of whitelisted emails on decoded identity tokens.
