...
## 2024-05-18 - Prevent Error Details Leak in API Responses
**Vulnerability:** API endpoints were returning internal error details (`error.message`) in HTTP API responses using Nuxt's `createError`.
**Learning:** This exposes internal logic and potentially sensitive data (like stack traces, db structure) to clients and attackers.
**Prevention:** Log errors server-side (e.g. `console.error`) and return a generic 'Internal Server Error' string to the client.
