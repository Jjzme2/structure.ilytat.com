...
**Vulnerability:** Document storage lacked access controls, allowing users to list or download any file by guessing its key.
**Learning:** Flat storage (like S3/R2) requires explicit scoping (e.g., `documents/users/${uid}/`) in list and download endpoints to prevent Insecure Direct Object References (IDOR).
**Prevention:** Always scope resource paths using the authenticated user's ID and validate access on retrieval.
