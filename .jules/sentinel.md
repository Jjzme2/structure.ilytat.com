...
**Vulnerability:** `requireAdmin` authorized users based solely on `user.email` matching an admin allowlist, without checking `user.email_verified`.
**Learning:** If an identity provider (like Firebase Auth) allows creating accounts with unverified emails, checking the email address alone allows malicious actors to impersonate admins.
**Prevention:** Always check `user.email_verified === true` when asserting privileges based on hardcoded or external email lists.
