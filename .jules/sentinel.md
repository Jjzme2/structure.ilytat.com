...
## 2026-02-08 - Information Disclosure via Server Errors
**Vulnerability:** API routes returned raw internal error messages (`e.message`) to the client via `createError` on 500 status codes.
**Learning:** Developers sometimes leak stack traces or raw error strings to the client to make debugging easier, but this exposes internal implementation details (e.g., Firebase Auth or Firestore internals) that can aid attackers.
**Prevention:** Always log detailed error messages server-side (`console.error`), and only return generic, safe error messages to clients (e.g., "Internal Server Error").
