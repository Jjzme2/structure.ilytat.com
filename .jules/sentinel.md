...
## 2024-05-18 - IDOR in Document Uploads/Downloads
**Vulnerability:** Document uploads were saved in a flat `documents/` structure, and the download endpoint did not verify if the requesting user owned the document, allowing IDOR.
**Learning:** S3 storage without a database mapping requires careful key structuring to enforce ownership.
**Prevention:** Scope user uploads to `documents/users/${uid}/` and explicitly validate the `uid` prefix in download endpoints while maintaining backward compatibility.
