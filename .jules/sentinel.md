...
## 2026-02-07 - Account Takeover via Unverified Email
**Vulnerability:** `server/utils/auth.ts` allowed access to hardcoded admin emails without explicitly checking if the user actually verified that email (`user.email_verified === true`).
**Learning:** When authorizing users based on hardcoded or custom email lists using identity providers like Firebase Auth, malicious users can create unverified accounts with those target emails and gain full access if verification state isn't checked.
**Prevention:** Always explicitly verify `user.email_verified === true` on the decoded token before trusting an email address for authorization purposes.
