...
## 2024-05-24 - Require verified emails for hardcoded admin lists
**Vulnerability:** Admin access could be granted to unverified accounts matching hardcoded admin email lists.
**Learning:** Identity providers like Firebase Auth allow creating accounts with any email, so checking email strings for authorization requires explicit `email_verified === true` checks to prevent account takeover.
**Prevention:** Always verify `user.email_verified === true` when authorizing based on custom or hardcoded email lists.
