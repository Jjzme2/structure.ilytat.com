...
## 2024-05-09 - Missing email verification check in hardcoded auth bypass
**Vulnerability:** The legacy hardcoded admin email check authorized any token bearing a whitelisted email address, regardless of whether the email was verified. A malicious user could create an account with an admin's email address and gain admin privileges.
**Learning:** When using identity providers, authorization based on an email address must always ensure the email is verified, otherwise account takeover is possible via unverified emails.
**Prevention:** Always verify `user.email_verified === true` along with the email string match when asserting identity based on email address.
