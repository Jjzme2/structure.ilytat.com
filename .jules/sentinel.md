...
## 2024-05-29 - Missing Email Verification Check in Authorization
**Vulnerability:** Authorization bypassed via maliciously created unverified accounts matching hardcoded admin emails.
**Learning:** Identity providers like Firebase Auth allow users to create accounts with any email address. Relying solely on `user.email` without checking `email_verified === true` permits account takeover of admin privileges.
**Prevention:** Always explicitly verify `user.email_verified === true` when granting access based on email addresses.
