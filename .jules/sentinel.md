...
## 2025-05-18 - Prevent IDOR in Document Upload/Download
**Vulnerability:** Insecure Direct Object Reference (IDOR) where users could upload files globally and access other users' files since storage was flat.
**Learning:** A flat S3 bucket architecture without custom access controls intrinsically exposes objects if object keys are predictable or guessable. Backward compatibility for legacy files limits the possibility of purely denying access without migration mapping.
**Prevention:** Isolate user uploads with `documents/users/${uid}/` prefixes and enforce string prefix matching in download/list handlers.
