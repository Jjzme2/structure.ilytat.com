...
**Vulnerability:** Admin access could be granted to unverified accounts whose email matched a hardcoded whitelist.
**Learning:** When authorizing users based on hardcoded or custom email lists using identity providers like Firebase Auth, always explicitly verify `user.email_verified === true` on the decoded token to prevent account takeover from maliciously created unverified accounts.
**Prevention:** Always enforce `email_verified === true` when checking explicit email values in authorization middleware.
