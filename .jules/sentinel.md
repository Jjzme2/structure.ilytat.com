...
## 2024-05-30 - Fix Authorization Bypass for Unverified Admin Emails
**Vulnerability:** Authorization bypass via unverified admin emails in legacy support checks.
**Learning:** Hardcoded email checks did not verify `user.email_verified`. Malicious actors could create an unverified Firebase account with an admin email and gain administrative privileges.
**Prevention:** Always verify `user.email_verified === true` when authorizing based on custom email lists with identity providers like Firebase Auth.
