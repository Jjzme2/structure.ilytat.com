...
**Vulnerability:** Authorization logic using hardcoded emails from Firebase Auth tokens failed to explicitly check `email_verified`, allowing potential account takeover via maliciously registered, unverified accounts matching admin emails.
**Learning:** Checking `user.email` alone is unsafe because identity providers like Firebase allow anyone to create an account with any email address unless specifically restricted.
**Prevention:** Always verify `user.email_verified === true` when asserting authorization purely based on an email address claim from an authentication token.
