...
## 2026-02-07 - Information Disclosure via Server API Handlers
**Vulnerability:** API handlers exposed internal error details to clients by directly passing `e.message` in 500 error responses.
**Learning:** Returning detailed backend error messages to the frontend can expose sensitive system, configuration, or database structures to users or attackers.
**Prevention:** Always log detailed error messages server-side using `console.error` and return generic, non-specific error messages (e.g., 'An internal error occurred') via `createError`.
