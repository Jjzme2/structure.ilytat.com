...
## 2026-04-02 - O(N) filters in Template Loops
**Learning:** Running O(N) array functions like `.filter()` or `.find()` inside Vue template loops (e.g. inside a `v-for`) multiplies operations (O(N*M)) causing excessive re-renders and potential UI lockups.
**Action:** Pre-compute mappings using a `computed` Map to convert dynamic O(N) loop queries into O(1) Map lookups.
