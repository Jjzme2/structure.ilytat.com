...
## 2024-06-23 - Firestore Batch Writes
**Learning:** Using `Promise.all` for multiple independent Firestore writes can cause significant network overhead and latency.
**Action:** Always prefer `writeBatch` from 'firebase/firestore' for bulk updates, taking care to respect the maximum limit of 500 operations per batch by chunking updates accordingly.
