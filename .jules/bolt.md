...
## 2024-05-24 - Use writeBatch for concurrent Firestore writes
**Learning:** Using `Promise.all` to perform multiple concurrent individual `updateDoc` calls creates excessive network overhead and latency in Firestore. The app's architecture benefits significantly from combining these into a single network request using `writeBatch`.
**Action:** When updating multiple documents (like marking several messages as read), utilize `writeBatch` instead of processing concurrent individual writes.
