...
## 2025-06-05 - Batching Firestore Operations
**Learning:** Using `Promise.all` with individual `updateDoc` calls for multiple Firestore updates causes excessive network round-trips. Firestore's `writeBatch` allows batching these operations into a single network request.
**Action:** Replace `Promise.all` loops of `updateDoc` with `writeBatch` in Firestore operations when multiple documents are updated simultaneously, keeping in mind the strict 500 operation limit per batch.
