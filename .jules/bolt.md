...
## 2025-05-27 - Firestore Bulk Updates with writeBatch
**Learning:** Using `Promise.all` with individual `updateDoc` calls for bulk operations (like marking all messages as read) causes performance bottlenecks and massive network overhead by sending concurrent individual HTTP requests.
**Action:** Always use Firestore's `writeBatch` to bundle bulk operations into single network requests, remembering to chunk updates to respect Firestore's strict limit of 500 writes per batch.
