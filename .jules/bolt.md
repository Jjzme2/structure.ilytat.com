...
## 2025-05-27 - Firestore Batch Write Performance
**Learning:** Using `Promise.all` to perform multiple concurrent individual writes in Firestore creates significant network overhead and latency.
**Action:** Use `writeBatch` to combine multiple operations into a single network request, ensuring batches are chunked to a maximum of 500 operations per request to respect Firestore limits.
