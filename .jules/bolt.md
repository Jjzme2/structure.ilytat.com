...
## 2025-05-27 - Vue Template Loop Optimization
**Learning:** Repeated O(N) array filtering operations (like `.filter()`) inside Vue template loops cause significant performance degradation during re-renders, especially with large datasets, as they execute O(N) times for each loop iteration.
**Action:** Pre-compute a `Map` within a `computed` property to structure the data beforehand, enabling O(1) lookups during template rendering.
