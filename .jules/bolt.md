...
## 2025-05-27 - Firestore Batch Operations
**Learning:** Using `Promise.all` with individual `updateDoc` calls on multiple documents generates separate network requests, which creates significant overhead.
**Action:** Always utilize `writeBatch` from 'firebase/firestore' when mutating multiple documents (like marking messages read) to consolidate network requests.
