...
## 2024-05-28 - Firestore Query Execution in Promise.all
**Learning:** When fetching multiple independent Firestore resources in a composable like `useDaily`, and optimizing queries, it's critical to ensure all queries are executed (e.g., via `getDocs`) within `Promise.all` and properly destructured. A missed query leads to a runtime reference error.
**Action:** Double-check the `Promise.all` array contents and destructuring alignment when modifying parallel fetches.
