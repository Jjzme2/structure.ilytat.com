...
## 2025-05-27 - Firestore Network Optimization with writeBatch
**Learning:** Processing concurrent individual Firestore updates via `Promise.all` (like marking multiple messages as read) introduces unnecessary network overhead and connection latency. Combining these into a single transaction using `writeBatch(db)` is a massive performance gain for operations touching multiple documents simultaneously.
**Action:** Always prefer `writeBatch(db)` over `Promise.all` when applying identical update operations across multiple Firestore documents in a single logical action.
