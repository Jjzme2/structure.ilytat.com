...
**Learning:** Calling `updateDoc` repeatedly inside `Promise.all` (e.g., for marking many messages as read) causes parallel network requests that consume bandwidth and can trigger rate limiting.
**Action:** Use `writeBatch(db)` instead of concurrent individual writes to combine multiple operations into a single network roundtrip, significantly reducing latency and overhead.
