...
## 2024-05-28 - Array Sorting Performance
**Learning:** In Vue composables/computed properties, it's a common performance anti-pattern to sort a full array before filtering it (e.g. `[...items].sort().filter()`). Sorting is O(N log N) while filtering is O(N). Filtering first reduces the N that needs to be sorted.
**Action:** Always apply `.filter()` before `.sort()` to minimize the dataset size passed to the sorting algorithm.
