...
## 2025-06-07 - Firestore Batch Writes for Multiple Updates
**Learning:** When performing multiple document updates simultaneously (like marking all inbox messages as read), using `Promise.all` with individual `updateDoc` calls creates unnecessary network overhead.
**Action:** Use `writeBatch(db)` to bundle multiple document updates into a single network request, drastically reducing latency and improving efficiency.
