...
## 2024-05-24 - Vue Array Computation Optimization
**Learning:** Arrays were being sorted before filtering in computed properties, leading to an O(N log N) operation on the entire list instead of a subset.
**Action:** ALWAYS apply O(N) filtering operations before O(N log N) sorting operations on large collections.
