...
## 2026-06-26 - Firestore Batch Updates Over Concurrent Requests
**Learning:** Using `Promise.all` with individual `updateDoc` calls on multiple Firestore documents introduces significant latency and network overhead.
**Action:** Replace concurrent individual writes with `writeBatch(db)` and `batch.commit()`, chunked into arrays of 500 or less, to combine operations into single requests.
