...
## 2024-05-27 - Firestore Batch Writes for Multiple Updates
**Learning:** Updating multiple documents in Firestore concurrently using `Promise.all` with individual `updateDoc` calls creates high network latency and excessive overhead, as each operation results in a separate network request.
**Action:** Always use Firestore's `writeBatch` (from `firebase/firestore`) when updating multiple documents. Ensure the updates are chunked into arrays of 500 or fewer to comply with Firestore's hard limits on batch operations.
