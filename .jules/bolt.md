...
## 2024-05-28 - Batching Firestore Bulk Updates
**Learning:** Performing multiple independent `updateDoc` operations (even in parallel with `Promise.all`) creates unnecessary network overhead and risk of rate limits for bulk actions.
**Action:** Always use `writeBatch` when modifying multiple documents simultaneously to reduce network roundtrips to a single request.
