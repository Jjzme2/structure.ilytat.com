...
## 2024-05-29 - Firestore Batch Writes for Multiple Updates
**Learning:** When updating multiple documents (e.g., marking several messages as read), processing concurrent individual writes via `Promise.all` adds network overhead.
**Action:** Use `writeBatch` from 'firebase/firestore' to combine multiple operations into a single network request, drastically reducing latency.
