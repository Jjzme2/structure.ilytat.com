...
## 2025-06-05 - Batching Independent Promises
**Learning:** A sequential `await getDocs()` operation after an existing `Promise.all()` creates a waterfall, degrading performance when fetching independent collections.
**Action:** Group all independent Firestore reads (e.g., `getDoc`, `getDocs`) into a single `Promise.all` array to maximize concurrent network requests and minimize TTFB.
