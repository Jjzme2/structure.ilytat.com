...
## 2026-04-14 - Array Filter Optimization in Vue Computeds
**Learning:** In Vue computed properties that process large arrays, applying `.filter()` after an expensive operation like `.sort()` (which is O(N log N)) wastes CPU cycles sorting elements that will immediately be discarded.
**Action:** Always apply O(N) operations like `.filter()` *before* expensive O(N log N) operations like `.sort()` when deriving data in Vue computed properties.
