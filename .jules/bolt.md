...
## 2025-05-27 - Vue Array Optimization: Filter Before Sorting
**Learning:** In computed properties, applying `.sort()` before `.filter()` results in unnecessary sorting of items that will be discarded. Sorting is O(N log N), so reducing the array size first via an O(N) filter provides a measurable performance gain.
**Action:** Always apply `.filter()` before `.sort()` when computing derived arrays.
