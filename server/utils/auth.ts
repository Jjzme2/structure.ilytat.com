...
export const requireAdmin = async (event: any) => {
    ...
    // Security: explicitly require email verification to prevent account takeover from unverified signups
    const adminEmails = ['jj@ilytat.com', 'admin@ilytat.com', 'zettler.jj@ilytat.com'];
    if (user.email && adminEmails.includes(user.email) && user.email_verified === true) {
        return user;
    }
