<template>
  <div class="min-h-screen space-y-8 pb-20 animate-fade-in">
        ...
        <div class="relative group/export">
          ...
          >
            ...
            <div class="p-2 space-y-1">
              <div
                v-for="fmt in ['json', 'yaml', 'xml', 'md', 'txt']"
                :key="fmt"
                class="flex items-center justify-between p-2 rounded-xl hover:bg-slate-800/50 transition-colors group/row"
              >
                <span
                  class="text-xs font-mono font-bold text-slate-400 group-hover/row:text-white"
                  >{{ fmt.toUpperCase() }}</span
                >
                <div class="flex gap-1">
                  <button
                    @click="exportTasks(fmt as any, 'download')"
                  ...
                  <button
                    @click="exportTasks(fmt as any, 'copy')"
    ...
    <div v-if="viewMode === 'list'" class="space-y-4">
          ...
          <div class="flex-grow min-w-0">
            <div class="flex items-center gap-3">
              <input
                ...
                :class="{
                  'line-through text-slate-500': task.status === 'done',
                }"
              />
              <span
                v-if="task.category"
              ...
              >
                {{ getCategoryInfo(task.category)?.emoji }}
            ...
            <p
              ...
              class="text-slate-400 text-sm mt-1 line-clamp-2"
            >
              {{ task.description }}
            </p>
            <p
              class="text-[10px] font-mono text-slate-600 uppercase tracking-widest mt-2"
            >
              {{
                task.status === "done"
                  ? "Completed"
      ...
      <div
        v-if="filteredListTasks.length === 0"
        class="text-center py-24 group"
      >
        <div
        ...
        >
          <svg
            class="w-10 h-10 text-slate-600 group-hover:text-indigo-500"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
    ...
    >
        ...
        >
          ...
          >
            <div class="flex items-start justify-between gap-2 mb-2">
              ...
              >
                <button
                  @click="handleAddToFocus(task.id)"
      ...
      <div class="space-y-4">
        ...
        >
          ...
          >
            <div class="flex items-start justify-between gap-2 mb-2">
              <div>
                <span class="text-xs font-bold text-purple-400 mr-2"
                  >#{{ task.focusOrder }}</span
                >
              ...
              >
                <button
                  @click="handleMoveToDoing(task.id)"
                ...
                <button
                  @click="handleRemoveFromFocus(task.id)"
      ...
      <div class="space-y-4">
          ...
          >
            <div class="flex items-start justify-between gap-2 mb-2">
              ...
              >
                ...
                <button
                  @click="handleMoveBack(task.id)"
                  aria-label="Move Back to Focus"
                  class="p-1.5 rounded-lg bg-slate-600/20 text-slate-400 hover:bg-slate-600/40 text-xs"
                  title="Back"
                >
                  ↩️
        ...
        <p
          v-if="doingTasks.length === 0"
          class="text-center py-8 text-amber-700 text-sm"
        >
          Start working on a focus task
        </p>
      </div>
      ...
      <div class="space-y-4">
        ...
        <p
          ...
          class="text-center py-8 text-emerald-700 text-sm"
        >
...
const getLinkedOKRInfo = (
  okrId: string | null | undefined,
...
const filteredListTasks = computed(() => {
  ...
  // ⚡ Bolt Optimization: Filter before sorting to reduce O(N log N) sorting overhead
...
const backlogTasks = computed(() =>
  (tasks.value?.filter((t) => t.status === "backlog") || []).sort(
...
const doingTasks = computed(() =>
  (tasks.value?.filter((t) => t.status === "doing") || []).sort(
    (a, b) => (a.rank ?? 0) - (b.rank ?? 0),
  ),
);
...
const checkFocusLimit = (evt: any) => {
  ...
  if (evt.to.classList.contains("ring-1") && focusTasks.value.length >= 6) {
    // Focus col has ring-1
