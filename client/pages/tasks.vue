...
const filteredListTasks = computed(() => {
    ...
    // Pre-filter the array before sorting. Filtering is O(N) while sorting is O(N log N).
    // By reducing the array size first, we significantly decrease the number of comparisons
    // needed during the sort operation, especially when filtering out many 'done' tasks.
