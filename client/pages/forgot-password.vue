      ...
      <form v-if="!emailSent" @submit.prevent="handleResetPassword" class="mt-8 space-y-6">
        <div>
          ...
          <input id="email" v-model="email" type="email" required placeholder="Email address"
            :aria-invalid="!!error" :aria-describedby="error ? 'error-message' : undefined"
        ...
        >
          <div v-if="error" id="error-message" role="alert" aria-live="assertive" class="text-red-400 text-center text-sm">
            {{ error }}
          </div>
        </Transition>
