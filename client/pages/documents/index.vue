  ...
  <div class="min-h-screen space-y-12 pb-20">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        ...
        <button @click="toggleForm"
          ...
          <svg v-else class="w-6 h-6 text-rose-500 group-hover:rotate-90 transition-transform duration-300" fill="none"
            viewBox="0 0 24 24" stroke="currentColor">
        ...
        <button @click="toggleUpload"
          ...
          title="Upload File">
          <svg v-if="!showUpload"
            class="w-6 h-6 text-indigo-500 group-hover:scale-110 transition-transform duration-300" fill="none"
            viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <button @click="fetchDocuments"
          class="w-12 h-12 flex items-center justify-center rounded-xl bg-slate-900 border border-slate-800 text-slate-400 hover:text-indigo-400 hover:border-indigo-500/30 transition-all"
          title="Refresh">
    ...
    <Transition name="slide-up">
      <div v-if="showUpload" class="relative group">
        <div
          class="absolute -inset-1 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-[2rem] blur opacity-20 group-hover:opacity-30 transition duration-1000">
        </div>
    ...
    <div class="space-y-12">
      ...
      <!-- Briefings Section -->
      <div v-if="briefings.length > 0">
        <h3
          class="text-lg font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2 border-b border-slate-800 pb-2 mb-4">
          🧠 Intelligence Briefings
          <span class="text-xs font-normal text-slate-600 bg-slate-900 px-2 py-0.5 rounded-full">{{ briefings.length
            }}</span>
        </h3>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <BriefingCard v-for="briefing in briefings" :key="briefing.id" :briefing="briefing" />
        </div>
      </div>
      ...
      <template v-else>
        <div v-for="(files, category) in categorizedDocuments" :key="category" class="space-y-4">
          <h3
            class="text-lg font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2 border-b border-slate-800 pb-2">
            {{ category === 'Unsorted' ? '📂 General Files' : `📂 ${category}` }}
            <span class="text-xs font-normal text-slate-600 bg-slate-900 px-2 py-0.5 rounded-full">{{ files.length
              }}</span>
          </h3>

          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            <div v-for="doc in files" :key="doc.key"
              class="group relative bg-slate-900 border border-slate-800 rounded-xl p-4 hover:border-pink-500/30 transition-all hover:shadow-lg hover:shadow-pink-500/5">
              <div class="flex items-start justify-between">
                <div class="flex items-center gap-3 overflow-hidden">
                  <span class="text-3xl">📄</span>
                  <div class="flex flex-col min-w-0">
                    <span class="text-sm font-bold text-slate-200 truncate pr-2" :title="doc.filename">{{ doc.filename
                      }}</span>
                    <span class="text-xs text-slate-500">{{ formatBytes(doc.size) }}</span>
                  </div>
                </div>
                <div class="flex gap-2">
                  <button @click="openDocument(doc.key, true)"
                    class="p-2 text-slate-400 hover:text-indigo-400 bg-slate-800 rounded-lg hover:bg-slate-700 transition-colors"
                    title="View Document">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  ...
                  <button @click="openDocument(doc.key, false)"
                    class="p-2 text-slate-400 hover:text-white bg-slate-800 rounded-lg hover:bg-slate-700 transition-colors"
                    title="Download">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div
                class="mt-2 pt-2 border-t border-slate-800/50 flex justify-between items-center text-[10px] text-slate-600 font-mono uppercase tracking-wider">
        ...
        <div v-if="Object.keys(categorizedDocuments).length === 0 && documents.length === 0"
          class="text-center py-12 bg-slate-800/20 rounded-2xl border border-dashed border-slate-800">
      ...
      <div class="pt-8">
        <h3
          class="text-lg font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2 border-b border-slate-800 pb-2 mb-4">
          📝 Notes
          <span class="text-xs font-normal text-slate-600 bg-slate-900 px-2 py-0.5 rounded-full">{{ notes.length
            }}</span>
        </h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          <div v-for="note in notes" :key="note.id" class="group relative h-64">
            <div
              class="absolute -inset-0.5 bg-gradient-to-br from-pink-500/0 to-purple-500/0 group-hover:from-pink-500/20 group-hover:to-purple-500/20 rounded-2xl transition-all duration-500 blur-sm">
            </div>
            <div
              class="relative flex flex-col h-full bg-slate-900 border border-slate-800 hover:border-slate-700 rounded-2xl p-6 transition-all duration-500 overflow-hidden">

              <div class="flex justify-between items-start mb-2">
                <h4 class="text-lg font-bold text-slate-200 line-clamp-1 group-hover:text-pink-400 transition-colors">{{
                  note.title }}</h4>
          ...
          <div v-if="notes.length === 0"
            class="flex flex-col items-center justify-center p-8 border border-dashed border-slate-800 rounded-2xl bg-slate-900/30 text-slate-500">
...
import type { Note, Briefing } from '~/types'
...
onMounted(() => {
  if (user.value) {
    fetchDocuments()
    // Briefings are auto-fetched by useCollection in the store, but we can ensure it's Init if needed
  }
})
