...
export const useInbox = () => {
    ...
    const markAllRead = async () => {
        if (!user.value) return
        const uid = user.value.uid // Extract to constant to satisfy TypeScript narrowing in closure
        const unreadMessages = inbox.value.filter(m => !m.read)
        if (unreadMessages.length === 0) return

        // ⚡ Bolt Optimization: Use writeBatch to group multiple updates into a single network request
        // drastically reducing latency and network overhead compared to Promise.all
