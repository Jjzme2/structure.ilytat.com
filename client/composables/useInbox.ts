...
export const useInbox = () => {
    ...
    const markAllRead = async () => {
        if (!user.value) return
        const messages = inbox.value.filter(m => !m.read)

        // ⚡ Bolt: Use writeBatch instead of Promise.all to reduce network overhead
        // and execute all updates in a single network request.
