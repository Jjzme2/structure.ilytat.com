...
export const useDaily = () => {
    ...
    const fetchDaily = async () => {
        ...
        try {
            ...
            // OPTIMIZATION: added tasksQuery to Promise.all to avoid waterfall or missing data
            const [dailySnap, datesSnap, metaSnap, tasksSnap] = await Promise.all([
                getDoc(dailyRef),
                getDocs(datesQuery),
                getDoc(metaRef),
                getDocs(tasksQuery)
            ...
            if (dailySnap.exists()) {
                ...
                // OPTIMIZATION: Limit to 50 recent quotes to avoid fetching thousands of documents
                ...
                if (!userQuotesSnap.empty) {
                    ...
                    // OPTIMIZATION: Limit system quotes as well
                    const systemQuotesQuery = query(
                        collection(db, 'quotes'),
                        where('userId', '==', 'system'),
                        limit(50)
                    )
            ...
            const focusTasks = todayTasks.sort((a, b) => (a.focusOrder || 0) - (b.focusOrder || 0))
