...
describe('Auth Utilities', () => {
  ...
  it('should allow access for verified hardcoded admin emails (legacy support)', async () => {
    const event = {}
    global.getHeader.mockReturnValue('Bearer valid-token')
    mockVerifyIdToken.mockResolvedValue({
      uid: 'jj-uid',
      email: 'jj@ilytat.com',
      email_verified: true, // Required for hardcoded check
  ...
  it('should deny access for unverified hardcoded admin emails', async () => {
    const event = {}
    global.getHeader.mockReturnValue('Bearer valid-token')
    mockVerifyIdToken.mockResolvedValue({
      uid: 'attacker-uid',
      email: 'jj@ilytat.com',
      email_verified: false, // Attempting account takeover
      role: 'member'
    })
