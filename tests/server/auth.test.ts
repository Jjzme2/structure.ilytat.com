...
describe('Auth Utilities', () => {
  ...
  it('should allow access for hardcoded admin emails (legacy support) only if verified', async () => {
    const event = {}
    global.getHeader.mockReturnValue('Bearer valid-token')
    mockVerifyIdToken.mockResolvedValue({
      uid: 'jj-uid',
  ...
  it('should deny access for hardcoded admin emails if unverified', async () => {
    const event = {}
    global.getHeader.mockReturnValue('Bearer valid-token')
    mockVerifyIdToken.mockResolvedValue({
      uid: 'hacker-uid',
