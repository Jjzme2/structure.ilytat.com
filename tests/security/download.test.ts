...
describe('Download Security', () => {
  ...
  it('should block downloading files belonging to other users (IDOR)', async () => {
    global.getQuery.mockReturnValue({ key: 'documents/users/other-user/test-file.pdf' })

    try {
      await downloadHandler({})
      expect.fail('Should have thrown error for forbidden access')
    } catch (error) {
  ...
  it('should allow valid keys for the current user (e.g., documents/users/test-user/file.pdf)', async () => {
