...
describe('Download Security', () => {
  ...
  it('should block IDOR attempts to access other users files', async () => {
  ...
  it('should allow valid keys for own files (e.g., documents/users/test-user/test-file.pdf)', async () => {
  ...
  it('should allow legacy non-user-scoped documents for backward compatibility', async () => {
    global.getQuery.mockReturnValue({ key: 'documents/legacy-file.pdf' })
    ...
    expect(callArgs.Key).toBe('documents/legacy-file.pdf')
    expect(callArgs.Bucket).toBe('test-bucket')
  })
})
